...
| 10 | 回顾模式 | 由 `vars.REVIEW_MODE`（`"true"`/`"1"`）开启：全局比赛起止时间不生效（题目内 `time` 条件保留原语义）；关闭排行榜/通过率与 rank 广播；玩家可自行重置游戏进度（`POST /api/reset-progress`，清空 `records`/`score_events`/`problem_state`/`game_storage` 并归零 users 冗余字段，账号保留）；公开 `GET /api/mode` 供前端感知。账号生命周期：不再提供玩家自助删号，改由 Cron Trigger 每日清理 180 天未登录的非管理员账号（`users.last_login_at`，存量以 `created_at` 为宽限起点） |
...
| POST | `/api/reset-progress` | 重置自己的游戏进度（仅回顾模式；清空 records/score_events/problem_state/game_storage，保留账号） |
...
- [x] **P7 回顾模式**：`vars.REVIEW_MODE` 开关；解除全局比赛时间限制、关闭排行榜/通过率与 rank 广播、玩家可自行重置游戏进度；Cron Trigger 每日清理 180 天未登录的非管理员账号
